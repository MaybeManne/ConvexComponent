...
export const runWorkflowStep = internalAction({
  ...
  handler: async (ctx, args): Promise<void> => {
    ...
    const task = await ctx.runQuery(internal.workflowMutations.getTaskInternal, {
      taskId: args.taskId,
    });
    ...
    if (elapsed > timeoutMs) {
      ...
      await ctx.runMutation(internal.workflowMutations.markTaskFailed, {
        taskId: args.taskId,
        error: {
    ...
    if (task.canceledAt) {
      ...
      if (task.providerTaskId) {
        try {
          await ctx.runAction(internal.provider.providerCancelTask, {
      ...
      await ctx.runMutation(internal.workflowMutations.markTaskCanceled, {
        taskId: args.taskId,
      });
      return;
    }
    ...
    if (task.status === "queued") {
      await ctx.runMutation(internal.workflowMutations.markTaskRunning, {
        taskId: args.taskId,
      });
    }
    ...
    if (!providerTaskId) {
      try {
        ...
        const startResult = await ctx.runAction(internal.provider.providerStartTask, {
          prompt: task.prompt,
        ...
        await ctx.runMutation(internal.workflowMutations.updateTaskProvider, {
          taskId: args.taskId,
          providerTaskId,
          cursor,
    ...
    try {
      const pollResult = await ctx.runAction(internal.provider.providerGetTask, {
      ...
      if (pollResult.logs.length > 0 || pollResult.screenshots.length > 0) {
        await ctx.runMutation(internal.workflowMutations.appendProgress, {
          taskId: args.taskId,
      ...
      if (pollResult.status === "succeeded") {
        ...
        await ctx.runMutation(internal.workflowMutations.markTaskSucceeded, {
          taskId: args.taskId,
      ...
      await ctx.scheduler.runAfter(pollIntervalMs, internal.workflow.runWorkflowStep, {
        taskId: args.taskId,
...
export const startWorkflow = internalAction({
  ...
  handler: async (ctx, args): Promise<void> => {
    ...
    await ctx.scheduler.runAfter(0, internal.workflow.runWorkflowStep, {
      taskId: args.taskId,
      startTime: Date.now(),
    });
  },
