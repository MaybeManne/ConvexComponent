...
/**
 * Get the Browser Use Cloud API key from environment.
  ...
  if (!key) {
    throw new Error(
      "BROWSER_USE_API_KEY is not set. " +
        "Set it via: npx convex env set BROWSER_USE_API_KEY <your_key>"
    );
  }
  return key;
}
...
export const providerGetTask = internalAction({
  ...
  handler: async (_, args): Promise<ProviderGetResponse> => {
    ...
    // Add completion event for succeeded status
    if (status === "succeeded") {
    ...
    // Add error details for failed status
    if (status === "failed") {
      result.error = {
